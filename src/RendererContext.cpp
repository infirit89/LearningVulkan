...
VkResult CreateDebugUtilsMessanger(
    VkInstance instance,
    const VkDebugUtilsMessengerCreateInfoEXT *messangerCreateInfo,
...
RendererContext::RendererContext(std::string_view applicationName) {
  ...
  m_Swapchain = new Swapchain(m_LogicalDevice, window->GetWidth(),
                              window->GetHeight(), PresentMode::Mailbox);
  ...
  for (size_t i = 0; i < m_Swapchain->GetImageViews().size(); i++) {
    ...
    Framebuffer *framebuffer =
        new Framebuffer(m_RenderPass, attachments, extent.width, extent.height);
  ...
  m_TransientTransferCommandPool =
  ...
  m_TransientGraphicsCommandPool =
      CreateCommandPool(VK_COMMAND_POOL_CREATE_TRANSIENT_BIT,
...
const PhysicalDevice *RendererContext::GetPhysicalDevice() const {
...
const std::vector<Framebuffer *> &RendererContext::GetFramebuffers() const {
  return m_Framebuffers;
}

const PerFrameData &RendererContext::GetPerFrameData(size_t index) const {
...
const VkPipeline &RendererContext::GetGraphicsPipeline() const {
  return m_Pipeline;
}
...
void RendererContext::CreateSurface() {
  const Window *window = Application::Get()->GetWindow();
  assert(glfwCreateWindowSurface(m_Instance, window->GetNativeWindow(), nullptr,
...
RendererContext::CreateStackCommandBuffer(VkCommandPool commandPool) {
  ...
  assert(vkAllocateCommandBuffers(m_LogicalDevice->GetVulkanDevice(),
                                  ...
                                  &vulkanCommandBuffer) == VK_SUCCESS);
...
CommandBuffer *RendererContext::CreateCommandBuffer(VkCommandPool commandPool) {
  ...
  CommandBuffer *commandBuffer =
      new CommandBuffer(commandPool, std::move(vulkanCommandBuffer));
  return commandBuffer;
}
...
void RendererContext::RecordCommandBuffer(uint32_t imageIndex,
  ...
  renderPassInfo.framebuffer =
      m_Framebuffers.at(imageIndex)->GetVulkanFramebuffer();
...
void RendererContext::CreateSyncObjects(
  ...
  assert(vkCreateFence(m_LogicalDevice->GetVulkanDevice(), &fenceCreateInfo,
                       nullptr, &presentFence) == VK_SUCCESS);
}
...
void RendererContext::CreatePerFrameObjects(uint32_t frameIndex) {
  ...
  data.CameraUniformBuffer =
      new GPUBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, sizeof(CameraData),
...
void RendererContext::DrawFrame() {
  ...
  submitInfo.pCommandBuffers =
      &currentFrameData.CommandBuffer->GetVulkanCommandBuffer();
...
void RendererContext::CreateGraphicsPipeline() {
  ...
  std::vector<char> vertexShaderData =
      ReadShaderFile("assets/shaders/bin/BasicVert.spv");
  std::vector<char> fragmentShaderData =
      ReadShaderFile("assets/shaders/bin/BasicFrag.spv");
...
#pragma endregion
  ...
  assert(vkCreateGraphicsPipelines(
             ...
             &graphicsPipelineCreateInfo, nullptr, &m_Pipeline) == VK_SUCCESS);
...
VkShaderModule
RendererContext::CreateShader(const std::vector<char> &shaderData) {
...
void RendererContext::CreateVertexBuffer() {
  ...
  CommandBuffer transferCommandBuffer =
      CreateStackCommandBuffer(m_TransientTransferCommandPool);
...
void RendererContext::CreateCameraDescriptorSetLayout() {
  ...
  assert(vkCreateDescriptorSetLayout(
             ...
             nullptr, &m_CameraDescriptorSetLayout) == VK_SUCCESS);
}
...
static glm::mat4 testlookat(glm::vec3 position, glm::vec3 direction,
  ...
  glm::vec3 right =
      glm::normalize(glm::cross(direction, glm::normalize(worldUp)));
...
void RendererContext::CreateTexture() {
  ...
  CommandBuffer transferCommandBuffer =
      CreateStackCommandBuffer(m_TransientTransferCommandPool);
  transferCommandBuffer.Begin(CommandBufferUsage::OneTimeSubmit);
  transferCommandBuffer.TransitionLayout(m_TestImage,
                                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  transferCommandBuffer.CopyBufferToImage(&stagingBuffer, m_TestImage, width,
                                          height);
  transferCommandBuffer.End();
  ...
  CommandBuffer graphicsCommandBuffer =
      CreateStackCommandBuffer(m_TransientGraphicsCommandPool);
  graphicsCommandBuffer.Begin(CommandBufferUsage::OneTimeSubmit);
  graphicsCommandBuffer.TransitionLayout(
      m_TestImage, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
